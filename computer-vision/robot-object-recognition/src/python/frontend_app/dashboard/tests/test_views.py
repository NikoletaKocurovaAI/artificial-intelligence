...
class ViewsUnitTestCase(TestCase):
    ...
    def test_login_user(self) -> None:
        def test_logout_user(self):
            ...
            self.assertEqual(response.status_code, 200)  # Assuming the home page returns 200 status code
            self.assertNotIn('_auth_user_id', self.client.session)  # Check that the user is not in the session
